...
async function computeBreakdown(
	...
	// Parse files with limited concurrency; aggregation itself stays on the main
	// loop between awaits, so addSessionToRange never interleaves.
	let parsedFiles = 0;
	let nextIndex = 0;
	const worker = async (): Promise<void> => {
		while (nextIndex < candidates.length) {
			if (signal?.aborted) return;
			const filePath = candidates[nextIndex++]!;
		...
		handler: async (_args, ctx: ExtensionContext) => {
			...
			const data = await ctx.ui.custom<BreakdownData | null>((tui, theme, _kb, done) => {
				...
				computeBreakdown(loader.signal, (update) => Object.assign(progress, update))
					...
					.catch((err) => {
						stopTicker();
						console.error("usage: failed to analyze sessions", err);
