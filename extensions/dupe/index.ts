...
async function createForkedSession(ctx: ExtensionContext): Promise<string | undefined> {
...
export default function (pi: ExtensionAPI): void {
	pi.registerCommand("dupe", {
		description: "Duplicate this session into a new Ghostty split, optionally with a prompt",
		handler: async (args, ctx) => {
			if (process.platform !== "darwin") {
				ctx.ui.notify("/dupe currently requires macOS (Ghostty AppleScript).", "warning");
				return;
			}
			...
			if (result.code !== 0) {
				...
				if (forkedSessionFile) {
					ctx.ui.notify(`Duplicated session was created: ${forkedSessionFile}`, "info");
				}
				return;
			}
			...
			if (forkedSessionFile) {
				...
				ctx.ui.notify(`Duplicated to ${fileName} in a new Ghostty split${suffix}.`, "info");
				if (wasBusy) {
					ctx.ui.notify("Duplicated from current committed state (in-flight turn continues in original session).", "info");
				}
			} else {
				ctx.ui.notify("Opened a new Ghostty split (no persisted session to duplicate).", "warning");
			}
		},
	});
}
