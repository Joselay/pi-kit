...
The payload may instead be a Codex apply_patch style patch: *** Begin Patch, then *** Add File: / *** Update File: / *** Delete File: sections, then *** End Patch. Row scripts cannot create new files; use *** Add File: or the write tool for that.
...
const TOOL_PROMPT_GUIDELINES = [
	...
	"Codex apply_patch payloads (*** Begin Patch ... *** End Patch) are also accepted as the edit text payload; row scripts cannot create new files, so use *** Add File: or the write tool for new files.",
];
...
const unifiedEditSchema = {
	...
	properties: {
		text: {
			type: "string",
			description: "The row edit script or Codex apply_patch payload.",
		},
	},
} as any;
...
function classicEditRows(oldText: string, newText: string): string[] {
	const rows = ["@REPLACE"];
	for (const line of normalizeToLF(oldText).split("\n")) rows.push(`-${line}`);
	if (newText.length > 0) for (const line of normalizeToLF(newText).split("\n")) rows.push(`+${line}`);
	return rows;
}

// Fallback for models emitting pi's built-in edit schema ({path, oldText,
// newText} or {path, edits}); converted to an equivalent row script.  Whole-line
// matching still applies, so mid-line fragments may need a retry with full lines.
function classicEditToRowScript(args: Record<string, unknown>): string | undefined {
	...
	for (const edit of edits) {
		...
		rows.push(...classicEditRows(oldText, newText));
	}
	return rows.join("\n");
}
...
function prepareUnifiedArguments(args: unknown): UnifiedEditParams {
	...
	if (typeof args === "object" && args !== null && !Array.isArray(args)) {
		...
		const classic = classicEditToRowScript(args as Record<string, unknown>);
		if (classic !== undefined) return { text: classic };
	}
...
// The bg wraps the whole component, so it must stay constant while args are
// streaming: toggling pending<->success repaints every line of the block, and
// once the block top scrolls above the viewport each repaint forces a full
// clear-and-redraw of the terminal (visible flicker, ~15x/sec).
...
// While args are still streaming, cap the rendered diff height.  A preview
// taller than the terminal puts the component's top above the viewport, so
// every progressive update would force a full clear-and-redraw.  Showing the
// head keeps already-rendered lines stable; only the trailing count changes.
const STREAMING_PREVIEW_MAX_DIFF_LINES = 20;

function renderPreviewDiff(diff: string, argsComplete: boolean, theme: any): string {
...
function buildUnifiedEditCallComponent(
	...
	component.setBgFn(getUnifiedEditHeaderBg(component.preview, previewIsStale, component.settledError, argsComplete, theme));
	component.clear();
	...
	if ("error" in component.preview) {
		// A stale error came from older partial args; don't show it as current.
	...
	pi.registerTool({
		...
		renderCall(args, theme, context: RenderContext<UnifiedRenderState>) {
			...
			if (component.previewArgsKey !== key) {
				// Keep any previous preview visible while the new one builds.
				// Collapsing the diff on every streamed args chunk oscillates the
				// component height, which pushes changed lines above the viewport and
				// forces pi-tui into full clear-and-redraw cycles (visible flicker).
