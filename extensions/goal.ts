...
export default function goalExtension(pi: ExtensionAPI) {
	...
	function syncStatusRefresh(ctx: ExtensionContext): void {
		// setStatus stores a static string, so elapsed time needs an explicit refresh.
		const needsRefresh = ctx.hasUI && goal?.status === "active" && goal.tokenBudget === undefined;
		if (!needsRefresh) {
			stopStatusRefresh();
			return;
		}
		if (statusRefreshTimer !== null) return;
		statusRefreshTimer = setInterval(() => updateStatus(ctx), 1_000);
	}
	...
	pi.on("session_shutdown", async () => stopStatusRefresh());
