...
function hasNeedsAttentionVerdict(messageText: string): boolean {
	...
	for (const line of lines) {
		const inlineMatch = line.match(/^\s*(?:[-*+]\s+)?[*_`]{0,3}(?:overall\s+)?verdict\s*[*_`]{0,3}\s*:\s*[*_`]{0,3}\s*(.+)$/i);
	...
	for (let i = 0; i < lines.length; i++) {
		...
		} else if (!/^\s*[*_`]{0,3}(?:overall\s+)?verdict\s*[*_`]{0,3}\s*:?\s*[*_`]{0,3}\s*$/i.test(line)) {
			continue;
		}
...
async function hasPendingChanges(pi: ExtensionAPI): Promise<boolean> {
	// Check for staged or unstaged changes to tracked files. If git itself
	// fails, fail closed so we never checkout over a possibly dirty tree.
	const { stdout, code } = await pi.exec("git", ["status", "--porcelain"]);
...
async function buildReviewPrompt(
	...
	switch (target.type) {
		...
		case "baseBranch": {
			...
			// Function replacements so `$` sequences in data never act as
			// replacement patterns. Titles (untrusted) are substituted last so
			// a title containing a placeholder is not re-substituted.
		...
		case "commit":
			if (target.title) {
				...
				return COMMIT_PROMPT_WITH_TITLE.replace("{sha}", () => target.sha).replace("{title}", () => title);
			}
	...
	function parseArgs(args: string | undefined): ParsedReviewArgs {
		...
		switch (subcommand) {
			...
			case "branch": {
				...
				if (!branch) return { target: null, extraInstruction, error: "Usage: /review branch <name>" };
				return { target: { type: "baseBranch", branch }, extraInstruction };
			}

			case "commit": {
				const sha = parts[1];
				if (!sha) return { target: null, extraInstruction, error: "Usage: /review commit <sha> [title]" };
			...
			case "folder": {
				...
				if (paths.length === 0) return { target: null, extraInstruction, error: "Usage: /review folder <path> [path...]" };
				return { target: { type: "folder", paths }, extraInstruction };
			}

			case "pr": {
				const ref = parts[1];
				if (!ref) return { target: null, extraInstruction, error: "Usage: /review pr <number|url>" };
			...
			default:
				return {
					target: null,
					extraInstruction,
					error: `Unknown /review subcommand: ${subcommand}. Expected uncommitted, branch, commit, folder, or pr.`,
				};
		}
	}
