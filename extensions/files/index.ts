...
const isDirectoryAt = (inputPath: string): boolean => {
	try {
...
const getGitStatusMap = async (pi: ExtensionAPI, cwd: string): Promise<Map<string, GitStatusEntry>> => {
	...
	for (let i = 0; i < entries.length; i += 1) {
		...
		if ((status.startsWith("R") || status.startsWith("C")) && entries[i + 1]) {
			// In `-z` output the record is "XY PATH\0ORIG_PATH\0"; keep the new
			// path and skip the original one.
			i += 1;
		}
...
const getEditableContent = (target: FileEntry): EditCheckResult => {
	try {
		...
		return { allowed: false, reason: "File could not be read" };
	}
};
...
const editPath = async (ctx: ExtensionContext, target: FileEntry, content: string): Promise<void> => {
	...
	try {
		// Guard against clobbering writes that happened while the external
		// editor was open (e.g. the agent editing the same file).
		const onDisk = readFileSync(target.resolvedPath, "utf8");
		if (onDisk !== content) {
			ctx.ui.notify(`${target.displayPath} changed on disk while editing; not saving`, "error");
			return;
		}
...
const revealPath = async (pi: ExtensionAPI, ctx: ExtensionContext, target: FileEntry): Promise<void> => {
	...
	const isDirectory = target.isDirectory || isDirectoryAt(target.resolvedPath);
	let command = "open";
	let args: string[] = [];
...
const openDiff = async (pi: ExtensionAPI, ctx: ExtensionContext, target: FileEntry, gitRoot: string | null): Promise<void> => {
	...
	// On success the directory is left in place: VS Code reads the snapshot
	// asynchronously, so it must outlive this call.
