...
function getTodosDirLabel(): string {
	// Registration-time label: show the override verbatim rather than
	// resolving against process.cwd(), which can differ from the session cwd.
	const overridePath = process.env[TODO_PATH_ENV];
	if (overridePath && overridePath.trim()) {
		...
		entries
			...
			.map(async (entry) => {
				...
				try {
					...
					// The file's mtime approximates close time; created_at alone
					// would delete a long-lived todo the moment it is closed.
					const stats = await fs.stat(filePath);
					const createdAt = Date.parse(parsed.created_at);
					const lastActivity = Math.max(stats.mtimeMs, Number.isFinite(createdAt) ? createdAt : 0);
					if (lastActivity >= cutoff) return;
					const lockPath = getLockPath(todosDir, id);
					const lockStats = await fs.stat(lockPath).catch(() => null);
					if (lockStats && Date.now() - lockStats.mtimeMs <= LOCK_TTL_MS) return;
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	const todosDirLabel = getTodosDirLabel();
			...
			switch (action) {
				...
				case "create": {
					...
					const result = await withTodoLock(todosDir, id, ctx, async () => {
						// generateTodoId checked existence before the lock; a
						// concurrent create may have taken the id since.
						if (existsSync(filePath)) {
							return { error: `Todo id ${formatTodoId(id)} was taken concurrently; retry create` };
						}
						await writeTodoFile(filePath, todo);
						return todo;
					});
