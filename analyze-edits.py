...
Adapted from mitsuhiko/agent-stuff for this setup's edit tools.  Reports how
many times the edit tool was invoked via each mode:
  - rows: unified-edit row script ({text} payload with [file]/@op rows)
  - patch: unified-edit Codex-style patch ({text} starting with *** Begin Patch)
  - builtin(N): pi's built-in edit ({path, edits} with N edits)
  - classic: single {path, oldText, newText}

Also breaks down by file extension, with failure rates throughout.
"""
...
PATCH_PREFIXES = ("*** Add File: ", "*** Delete File: ", "*** Update File: ")
...
def classify_edit(args: dict) -> tuple[str, list[str]]:
    """Returns (mode_label, list_of_file_paths)."""
    text = args.get("text")
    if isinstance(text, str):
        ...
        if text.lstrip().startswith("*** Begin Patch"):
            ...
            return "patch", paths
        paths = [m.group(1) for line in lines if (m := re.fullmatch(r"\[(.+)]", line))]
        return "rows", paths

    if isinstance(args.get("path"), str):
        if isinstance(args.get("edits"), list):
            return f"builtin({len(args['edits'])})", [args["path"]]
        if "oldText" in args and "newText" in args:
            return "classic", [args["path"]]
...
def main():
    args = sys.argv[1:] or [os.path.expanduser("~/.pi/agent/sessions")]
